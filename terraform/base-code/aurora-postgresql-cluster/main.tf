...
resource "aws_rds_cluster_instance" "example_primary_instance" {
  count                = 2
...
resource "aws_route53_record" "cluster-alias" {
  count = length(aws_rds_cluster_instance.example_primary_instance)

  zone_id = "Z09063052B43KCQ7FSGHY"
  name    = "alpha-${count.index}"
  type    = "CNAME"
  ttl     = "30"

  records = [aws_rds_cluster_instance.example_primary_instance[count.index].endpoint]
}